<script>
  ...
  import { parse } from "svelte/compiler";
    ...
    function post_answer(event) 
        ...
        fastapi('post', url, params, (json) => {content = '' 
            error = {detail:[]} 
            get_question()},
    ...
    if(window.confirm('정말로 삭제하시겠습니까?'))
        ...
        fastapi('delete', url, params, (json) => {get_question()},
                (err_json) => {error = err_json} )
        }
    }
    ...
    function vote_question(_question_id) {
        if(window.confirm('정말로 추천하시겠습니까?')) {
            ...
            fastapi('post', url, params, 
                (json) => {get_question()}, (err_json) => {error = err_json} )
        }
    }
    ...
    function vote_answer(answer_id) {
        if(window.confirm('정말로 추천하시겠습니까?')) {
            ...
            let params = {answer_id: answer_id}
            fastapi('post', url, params, (json) => {get_question()},
                (err_json) => {error = err_json}
            )
        }
    }
...
<div class="container my-3">
            ...
            <div class="my-3">
                ...
                {#if question.user && $username === question.user.username}
                    <a use:link href="/question-modify/{question.id}"
                        class="btn btn-sm btn-outline-light bg-warning">수정(Update)</a>
                    <button class="btn btn-sm btn-outline-light bg-danger"
                        on:click={() => delete_question(question.id)}>삭제(Delete)</button>    
                {/if}
            </div>
        </div>
        </div>
    ...
    <!-- 답변 목록 -->
    <h5 class="border-bottom my-3 py-2" style="color: red">{question.answers.length}개의 답변이 있습니다.</h5>
